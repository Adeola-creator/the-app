...
function Card(props) {
    const {id, initial, firstName, lastName, department, lastVisit,children} = props
    return (
        <div className="relative py-8 px-3 h-60 items-center justify-center w-full max-w-[30ch] flex flex-col shadow-lg rounded-lg ">
            ...
            <div>
            <Link href={{
                pathname: `/patients/${encodeURIComponent(id)}`}} className="py-1 px-5 mt-20 text-xs font-bold text-white bg-[#007560] rounded">More</Link>
            </div>
           {children}
        </div>
    );
